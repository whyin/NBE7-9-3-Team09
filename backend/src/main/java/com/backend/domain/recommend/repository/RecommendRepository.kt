...
interface RecommendRepository : JpaRepository<Recommend?, Long?> {
    ...
    fun findByPlaceCategoryNameOrderByBayesianRatingDesc(categoryName: String?): List<Recommend?> // 카테고리별 추천 장소 조회

    fun findTop5ByPlaceCategoryNameOrderByBayesianRatingDesc(categoryName: String?): List<Recommend?> // 카테고리별 상위 5개 추천 장소 조회
